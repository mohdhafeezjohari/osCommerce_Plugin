...
if ( $nbcb=="1" ) {
    ...
    if ( ($skey==$key1) && ($status=="00") ) {
        ...
        $succ = 1;
        $comment = "(callback) NBePay payment : captured " . $order->info['comments'];
			
    //================================== PAYMENT FAILED ==================================
    }
    else {		 				
        $nb_order_status = 1;
        $comment = "(callback) NBePay payment : failed ".$order->info['comments'];
    }
    ...
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
        ...
        //tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array); // SAVED DURING NBEPAY PRE-ORDER
        ...
        if (isset($order->products[$i]['attributes'])) {
            ...
            for ($j=0, $n2=sizeof($order->products[$i]['attributes']); $j<$n2; $j++) {
                ...
                //tep_db_perform(TABLE_ORDERS_PRODUCTS_ATTRIBUTES, $sql_data_array); // SAVED DURING NBEPAY PRE-ORDER
    ...
    //else { tep_redirect( tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=nbepay&ErrDesc=nbepaypaymentfailed', 'SSL') ); }
 } 
?>